...
We are now moving into some more applications of the basic concepts we spent lectures 1-3 learning. [Good job](https://catoftheday.com) on making it this far, stuff gets a little more interesting (and difficult) here.
...
Now lets get into how to actually talk about linear transformations. A linear transformation is a set of operations that takes us from a domain in $$\mathbb{R^m}$$ to a domain in $$\mathbb{R^n}$$. This means that it takes us from one dimension to another.
...
Now lets check homogeneity. For this, the function of a scalar multiple of an input must equal the scalar multiple of the function value of that input.
...
Lets say matrix B represents the linear transformation $$ T^{-1} $$, which transforms vector _u_ to vector _v_. This means that if you apply B to the output, you get the input. Then:

$$
v = Bu
$$

So we can also define that matrix B is the inverse of matrix A, because it describes the inverse of the linear transformation given by A, as it reverses the effect. Thus:
...
(Note you can kind of make functions surjective by just redefining the output space to ignore the area the function doesn't map to, but we don't believe this is too important to understand for now.)
...
So if a function is bijective, that means that every input it has maps to exactly one output. That means if you knew the output, you could trace it back to the input.

But how do you trace it back? Well you do the opposite of what you did to go from input to output, and this opposite is what we call the __inverse__. 
...
A matrix is considered invertible if it meets the following conditions:

1. Matrix must be square
   - This should make sense since when solving for the inverse, we augment it with the identity matrix. If the matrix is not square, it cannot have an identity matrix with 1's along the diagonal for all rows
2. A matrix is invertible if and only if its determinant is not equal to 0 ($$\text{det}(A) \neq 0$$)
    - Determinants can only be solved for in square matrices which furthers the first point
    - This is another good way to quickly check for invertibility.
...
To put it briefly, inverses are vital because this gives us yet another tool we need in our kit to dive into the second half of linear algebra (which we still won't do yet because there are some loose ends to tie up.)


## Practice
Find a linear combination of $$ \begin{bmatrix} -1 \\ 2\\ \end{bmatrix} $$
 and $$ \begin{bmatrix} 1 \\ 1 \\ \end{bmatrix} $$
that makes $$ \begin{bmatrix} 1 \\ 2 \\ \end{bmatrix} $$ [1/3 4/3]

Is the function $$ e^x $$ linear? How can you prove this? No, by testing the two parts of linearity.

For the following functions, are they injective, surjective, or bijective?
...
CHECK IF SWITCHING ROWS IN INVERTING MATRIX matters
