...
**A set of vectors is considered linearly independent of each other such that:**
...
We can then prove this by placing the vectors into an **augmented matrix** and placing the matrix into **RREF**, to find the coeffcient values. We won't show all of the steps of the elimination however, here are the RREF for both of the example sets, to show if they are linearly indepedent.
...
For example, $$[1,0]$$ , $$[0,1]$$, $$[1,1]$$ is a spanning set for $$\mathbb{R^2}$$, because every 2-d vector can be described as a linear combination of these 3 vectors. We have seen spanning sets just barely introduced before, when we discussed free variables in solutions to systems of equations.

Lets look at this matrix and the solution space it describes, which we discussed earlier way back in lesson 3:
...
So here the solutions were arranged so that each variable is a function of the free variable, which we will now call _t_ (You can call it whatever you want). 
...
Now finally, what this means is that all solutions to this matrix (and the associated system of equations) has solutions that can be described as a combination of the vectors 
$$
\begin{bmatrix}
  -4 \\
  -3  \\
...
so these are the spanning vectors of the solution space. Thus we take a matrix, that initially described a system with infinitely many solutions, and we did calculations to find __just 2 vectors__ that we can use to describe the entire solution space. That's really powerful! We'll see more applicatons of spanning sets later on, and in the practice problems for this lecture.
...
Now we must talk about the image and kernel of a linear transformation. The image of a linear transformation is simply the entire subspace that the transformation maps to. The dimension of this subspace is known as the __rank__. This is essentially the "solution space". 

Here is a visual example of the image of a linear transformation:

The kernel is quite different however, and represents the entire area of the domain that a linear transformation maps to the zero vector. All linear transformations must have the 0 vector in their kernel by definition. The dimension of the kernel is known as the __nullity__.

Here is a visual example of the kernel of a linear transformation:
...
Please take your time to understand this, and if you are confused, reach out to the creators of bearable via our community page, as we are happy to help and receive feedback.
